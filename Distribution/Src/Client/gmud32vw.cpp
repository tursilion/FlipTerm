...
afx_msg LONG CMudView::OnAsynchGetHostCompleted(UINT,LONG lParam)
{
	...
	if(nError)
	{
		Printf("%%%%%% Error %d - %s -in DNS lookup of %s.\n",(int)
			nError,m_pSocket->SockerrToString(nError),m_pWorld->m_sHostName);
					...
					if (pMenu) {
						...
						if (pMenu) {
							...
							for (int idx=0; idx<nCnt; idx++) {
								pMenu->GetMenuString(idx, csTmp2, MF_BYPOSITION);
		...
		if (sStr[0]=='/') {
			...
			if (m_pWorld) {
				...
				if ((m_pWorld->m_bIsIRC) && (!m_pWorld->m_csIRCChannel.IsEmpty())) {
					if (!bSystem) {
						CString csTmp;
						csTmp.Format("PRIVMSG %s :%s", m_pWorld->m_csIRCChannel, (LPCSTR)sStr);
						sStr=csTmp;
					}
				}
			}
...
void CMudView::OnFileTogglelogging() 
{
	...
	else
	{
		...
		CString fn;
		fn.Format("%s\\%s.log", GetApp()->m_sLogFolder, m_pWorld->m_sName);
...
void CMudView::MCPInternal(const char *pInStr) 
{
	...
	pStr.Format("#$#mcp-internal %s type: %s", m_MCPKey, pInStr);
